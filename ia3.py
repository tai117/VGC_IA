...
VERSION_ACTUAL = "1.0"
URL_VERSION_GITHUB = "https://raw.githubusercontent.com/TU_USUARIO/TU_REPO/main/version.txt" # <-- CAMBIA ESTO
...
def verificar_actualizaciones():
    try:
        console.print("[dim]Verificando actualizaciones en el servidor...[/dim]")
        respuesta = requests.get(URL_VERSION_GITHUB, timeout=3)
        if respuesta.status_code == 200:
            version_nube = respuesta.text.strip()
            if version_nube != VERSION_ACTUAL:
                console.print(f"\n[bold red]¡Nueva versión disponible ({version_nube})![/bold red]")
                console.print("Por favor, descarga el nuevo ejecutable desde GitHub.")
                console.print("Cerrando el sistema para evitar conflictos de versión...")
                sys.exit()
    except Exception as e:
        console.print(f"[dim yellow]No se pudo verificar la versión (Modo Offline activo)[/dim yellow]")
