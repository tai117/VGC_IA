...
# Iniciamos el listener en modo Daemon para que no bloquee recursos
...
def solicitar_importacion(label="EQUIPO"):
    console.clear() # Método cross-platform optimizado
    console.print(Panel(Align.center(f"[bold cyan]📥 MODO IMPORTACIÓN: {label}[/bold cyan]"), border_style="cyan"))
    console.print("Pega el código Showdown y presiona [bold]ENTER dos veces[/bold] para confirmar:\n")
    
    lines = []
    while True:
        line = input().strip()
        # Bug de 'lineas' corregido aquí
